  ...
  return (
    <header className="flex items-center justify-between px-4 py-3 bg-white rounded-t-2xl shadow border-b border-gray-200">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-2 text-gray-700 font-medium">Hola {username}</span>
      </div>
      <nav className="flex-1 flex justify-center gap-8">
        ...
        {isAdmin && (
          <button className="text-gray-700 font-medium hover:text-blue-500 transition">Panel Admin</button>
        )}
      </nav>
      <button onClick={handleLogout} className="ml-4 px-3 py-1 rounded bg-red-100 text-red-600 hover:bg-red-200 transition flex items-center gap-2">
